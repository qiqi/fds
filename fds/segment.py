...
def compute(outputs):
    ...
    file_inputs = [x for x in graph.input_values if x not in [zero, random]]
    n = np.loadtxt(file_inputs[0].field).shape[0]
...
def run_segment(run, u0, V, v, parameter, i_segment, steps,
    ...
    for j in range(subspace_dimension):
        ...
        res_h.append(threads.apply_async(
            run, (u1h[j], parameter, steps, run_id, interprocess)))
    ...
    res_i = threads.apply_async(
            run, (u1i, parameter + epsilon, steps, run_id, interprocess))
