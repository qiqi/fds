...
MPI_NP = 8                      # number of MPI processes for each instance
...
REF_WORK_PATH = os.path.abspath(os.path.join(os.environ['HOME'], 'Working/cylinder_fds_ref'))
    ...
    while not os.path.exists(final_data_file) or \
        ...
        with open(outfile, 'w', 8) as f:
            ...
            # subprocess.call(['mpirun', '--host', ','.join(nodes.grabbed_nodes)
                           # , 'python', 'write_J'] 
                           # , cwd=work_path, stdout = f, stderr=f)
            subprocess.Popen(['mpiexec', '-n', str(MPI_NP), charles_bin],
                  cwd=work_path, stdout=f, stderr=f).wait()
            subprocess.Popen(['python', 'write_J.py'],
                  cwd=work_path, stdout=f, stderr=f).wait()
