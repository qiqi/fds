...
def save_compressible_les_normalized(fname, les0, verbose=True):
    # first normalize then save
    les = deepcopy(les0)
    nnn = sqrt(linalg.norm(les['RHO'])**2 + linalg.norm(les['RHOU'])**2 + linalg.norm(les['RHOE'])**2) # norm
    nnn /= sqrt(3* 7.5e5)
    les['RHO']          = les['RHO']            / nnn
    les['RHOU']         = les['RHOU']           / nnn
    les['RHOE']         = les['RHOE']           / nnn
    les['right:RHO_BC'] = les['right:RHO_BC']   / nnn
    les['right:P_BC']   = les['right:P_BC']     / nnn
    les['right:U_BC']   = les['right:U_BC']     / nnn
    save_les(fname, les, verbose)
...
K_SEGMENTS = 800                # number of time chunks
...
REF_WORK_PATH = os.path.abspath('/scratch/niangxiu/fds_4CLV_finer_reso/apps/ref')
