program driver
	...
	type(active), dimension(:), allocatable :: a
    type(active), dimension(:,:), allocatable :: b
	double precision, dimension(d) :: X0, Xorig
	...
	allocate(b(d,subspace_dimension))
	allocate(a(subspace_dimension))
	...
	do t = 1, subspace_dimension, 1
		...
		a(t)%v = eps
		call head_homogeneous(a(t)%v,b(:,t),s,X0,v0(:,t),nSteps,Dcheb)
