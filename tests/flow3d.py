...
def solve(u0, jet_V, nsteps, run_id):
    ...
    if not os.path.exists(os.path.join(solver_path, 'final_state.npy')):
...
if False:
    s = linspace(0.8, 1.2, 21)

    def run(si):
        u, _ = solve(u0, si, 200, 'runup_{0:8.6f}'.format(si))
        _, Ji = solve(u, si, 500, 'fd_{0:8.6f}'.format(si))
        return Ji
